...
namespace Pricat.Api.Middelware
{
    public static class ExceptionMiddlewareExtensions
    {
        public static ErrorDetails ConstructErrorMessages(this ActionContext context)
        {
            var allErrors = context.ModelState.Values.Where(v => v.Errors.Count >= 1)
                    .SelectMany(v => v.Errors)
                    .Select(v => v.ErrorMessage)
                    .ToList();

            var hasDeserializationError = allErrors.Any(e =>
            ...
            var filteredErrors = hasDeserializationError
                ? allErrors.Where(e => !e.Contains("field is required", StringComparison.OrdinalIgnoreCase)).ToList()
                : allErrors;
